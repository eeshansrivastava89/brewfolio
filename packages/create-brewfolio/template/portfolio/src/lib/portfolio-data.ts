...
export async function getPortfolioData() {
	const [projectEntries, writingEntries, notebookEntries, conceptsData, siteConfig, aboutData, timelineData, impactData] = await Promise.all([
...
export function substackTitlebar(name: string): string {
	return '<div class="modal-titlebar-title">from <strong>' + escapeHtml(name) + '</strong></div>'
}

export function subscribeButton(label: string, href: string): string {
